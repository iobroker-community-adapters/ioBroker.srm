                ...
                return new Promise(async (resolve, reject) => {
                    try {
                        const obj = await new Promise((res, rej) => {
                            harness.objects.getObject('system.adapter.srm.0', (err, o) => {
                        ...
                        const stateIds = await harness.dbConnection.getStateIDs('srm.0.*');
...
Since this adapter connects to a local Synology router, integration tests requiring real hardware are not run in CI. Use mock data or a test Synology instance when available.
...
## SRM-Specific Development Patterns
...
1. **Polling Intervals**: Balance data freshness with API load (default: 60 seconds)
...
1. **Credential Storage**: Use encrypted native storage for passwords (`encryptedNative` in io-package.json)
